...
and following it on the input, carrying out no target score; (2) rewrite once
by reading `vendor/humanizer/SKILL.md` and following it (voice discovery,
density pre-check, multi-pass workflow, meaning check); (3) re-diagnose once
on the after text, fresh and cold, for residual reporting only. Emit the exact
output contract: Before / Authenticity read (before) / After / What changed /
Residual / What remains is a human's call.
